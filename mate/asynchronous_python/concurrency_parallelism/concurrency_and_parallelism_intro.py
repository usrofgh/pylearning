...
def main_threads():
    ...
    for i in range(n):
        ...
        tasks[-1].start()  # стартуем каждый thread к-й я добавил, после старта удаляется з листа как понял

    # если бы не добавил что ниже, то функция main_threads закончилась бы раньше, чем выполнились бы все треды
    for task in tasks:
        task.join()  # дожидаемся выполнения таски, а не заканчивать функцию main_threads(главный поток)
#----------------------------------------------------------------------------------------------------------------------
# 2-й подход - работа с процессами


def main_processes():  # Так же как и выше, только с помощью процессов, а не потоков
    # нет смысла создавать больше процессов чем ядер. Не будет конкурентности, будет паралелизм, но с io block O
    tasks = []
...
async def async_send_request(num: int, url: str, client: httpx.AsyncClient):
    print(f"Sending request #{num}")
    response = await client.get(url)  # TODO: back to num
    print(response.status_code)


async def main_asyncio():  # в основном потоке
