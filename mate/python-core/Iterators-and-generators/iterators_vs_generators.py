...
# В итераторе сначала нужно создать экз.класса с переданнымы итерируемыми значениями и обвернуть это функцией iter(), дальше можно юзать next().
# В генераторе нужно просто передать итерируеммые значения в функцию, и юзать next
# В итераторе чтобы пройтись по новой по списку, нужно просто для экз. класса вызвать iter(обнуляет self.current_element). Для джен - по новой вызвать функцию
